...
Life OS умеет находить свежие материалы по четырём областям проекта в разрешённых RSS/Atom-источниках, ранжировать их по свежести и ключевым словам, а затем передавать лучшие URL в существующий конвейер Claude → Obsidian → SQLite → RAG.

Проверка без добавления заметок:

```text
python discover.py --dry-run
```

Ручной запуск с добавлением заметок:

```text
python discover.py
```

Чтобы запускать поиск на Windows автоматически каждые три дня в 07:30, один раз откройте PowerShell от имени администратора в папке проекта и выполните:

```powershell
powershell -ExecutionPolicy Bypass -File scripts/install_scheduler.ps1
```

Время можно изменить: `scripts/install_scheduler.ps1 -Time "08:00"`. Компьютер должен быть включён; параметр `StartWhenAvailable` запускает пропущенную задачу после включения. Источники, темы, ключевые слова, глубина поиска и лимит материалов на один запуск задаются в секции `discovery` файла `config.yaml`. Уже обработанные URL сохраняются локально в `data/discovery_state.json`.
