...
На Windows дважды щёлкните по `ЗАПУСТИТЬ_DASHBOARD.bat`: файл сам создаст окружение, установит зависимости и откроет дашборд в браузере.

Дашборд показывает новые и прочитанные материалы, избранное, поиск, фильтры по темам и типам, а также десять наиболее релевантных материалов недели.
...
После запуска браузер откроет локальную библиотеку Life OS. Данные берутся из SQLite и заметок Obsidian; отметки «прочитано» и «избранное» хранятся отдельно и не изменяют Markdown-файлы.
